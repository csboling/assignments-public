...
%    The assumption that $p$ does not divide $q-1$ means that
%    $q \not\equiv 1 \pmod{p}$, which means $N_p \neq q$ and thus $N_p
%    = 1$, so there is a normal $p$-Sylow in $G$. Call it $P$.
%    Then $P \triangleleft G$, $P \cap Q = \{ 1 \}$, and $G = PQ$, so
%    $G = P \rtimes_\psi Q$, where $\psi : Q \to \mathrm{Aut}(P)$
...
%   Since $n$ is prime, every
%   nonzero element $m \in \mathbb{Z}_n$ has a multiplicative inverse
%   (which is also nonzero) so that
%   \begin{align*}
%   1 &= m^{-1} m
%      = \sum_{i=1}^{m^{-1}} m \\
%     &= \sum_{i=1}^{m^{-1} - 1} m + m
...
%   \end{align*}
%   so multiplying by this inverse is the same as adding 1 a total of
%   $m(m^{-1} - 1)$ times. (Here the formula is computed in the
%   integers but using $m^{-1}$ from $\mathbb{Z}_n$, e.g. in
%   $\mathbb{Z}_5$ we have $2^{-1} = 3$, and $2(3 - 1)
%
